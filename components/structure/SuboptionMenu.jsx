...
const SuboptionMenu = ({ subOptions, urlPath }) => {
  return (
    <>
      ...
      <ul className="absolute top-11 -right-[34px] h-52 overflow-y-scroll rounded bg-[#f6f8fc] pt-3 text-[#070b16] shadow transition-all duration-[2000ms] ease-in-out">
        {subOptions.map((subOption, i) => (
          <Link href={`${urlPath}/${subOption.url}`} key={i}>
            <li
              className={`relative flex  items-center gap-2 py-2 font-medium hover:bg-blue-100/80 ${
